...
python3 -m verl.trainer.main_ppo \
    ...
    env.seed=4608 \
    +env.use_old_prompt=False \
    env.rollout.n=$group_size \
    env.sokoban.dim_room=[6,6] \
    env.sokoban.num_boxes=2 \
    env.sokoban.search_depth=100 \
    env.sokoban.mode='text_with_row_numbers' \
    env.num_attempts=1 \
    env.max_steps=30 \
    env.max_sol_steps=21 \
    env.num_actions_per_turn=3 \
    env.max_turns=7 \
    ...
    trainer.experiment_name='sokoban_gigpo_qwen3_4b' \
    ...
    2>&1 | tee -a ../sokoban_gigpo_qwen3_4b.log