...
class TrajectoryCollector:
    ...
    def vanilla_multi_turn_loop(
        ...
        traj_logs = envs.get_traj_log()
        traj_cot_logs = self.get_traj_cot_logs(total_batch_list)
        ...
        return total_batch_list, episode_rewards, episode_lengths, success, traj_uid, traj_logs, traj_cot_logs
    ...
    def multi_turn_loop(
        ...
        if self.config.algorithm.filter_groups.enable and is_train:
            ...
            raise "#Todo on filtering traj_logs and traj_cot_logs"
            total_batch_list, total_episode_rewards, total_episode_lengths, total_success, total_traj_uid, traj_logs, traj_cot_logs = \
        ...
        if is_train:
            ...
            return gen_batch_output, traj_logs, traj_cot_logs

    def get_traj_cot_logs(self, total_batch_list):
        '''
        ...
        Returns:
            traj_cot_logs (List[String]): List of trajectory text
        '''
        traj_cot_logs = []
        for traj_batch in total_batch_list:
            cot_log = {'input': '', 'reward': 0}
            for i, step in enumerate(traj_batch):
                ...
                if i == 0: 
                    if 'multi_modal_inputs' in step.keys():
                        cot_log['image'] = Image.fromarray(step['anchor_obs'].astype(np.uint8))   
                    # ignore system prompt    
                    cot_log['input'] = input_text.split('user')[-1]
                
                if step['active_masks']:
                    cot_log[f'action_{i}'] = text_action
                    cot_log['reward'] = step['rewards']
                else:
                    cot_log[f'action_{i}'] = ''
