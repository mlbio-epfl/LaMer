    ...
    def _fetch(
        ...
        Returns:
            ...
            valid_lengths : List[int]
                A list of the actual number of valid history steps per environment.
        """
        memory_contexts, obs_contexts, action_contexts, valid_lengths = [], [], [], []
        ...
        for env_idx in range(self.batch_size):
            ...
            obs_lines, action_lines, context_lines = [], [], []
            for j, rec in enumerate(recent):
                step_num = start_idx + j + 1
                act = rec[action_key]
                obs = rec[obs_key]
                obs_lines.append(f"Observation {step_num}: '{obs}'")
                action_lines.append(f"Step {step_num}: '{act}'")
                context_lines.append(f"[Observation {step_num}: '{obs}', Step {step_num}: '{act}']")
        ...
        return obs_contexts, action_contexts, memory_contexts, valid_lengths