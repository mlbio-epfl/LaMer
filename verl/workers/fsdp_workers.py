...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_sequences_agent(self, prompts: DataProto):
        # Support all hardwares
        prompts = prompts.to(get_torch_device().current_device())
        ...
        with self.rollout_sharding_manager:
            ...
            prompts = self.rollout_sharding_manager.preprocess_data(prompts)
            output = self.rollout.generate_sequences_agent(prompts=prompts)
    ...
    # @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    # def _enter_rollout_ctx(self):
    #     # 用 ExitStack 兼容重复进入
    #     if not hasattr(self, "_ctx_stack"):
    #         from contextlib import ExitStack
    #         self._ctx_stack = ExitStack()
    #     self._ctx_stack.enter_context(self.rollout_sharding_manager)

    # @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    # def _exit_rollout_ctx(self):
    #     if hasattr(self, "_ctx_stack"):
    #         self._ctx_stack.close()   # 等价于 __exit__
    #         del self._ctx_stack
...
class CriticWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_prompt_values(self, data: DataProto):
        # Support all hardwares
        data = data.to(get_torch_device().current_device())
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            values = self.critic.compute_prompt_values(data=data)
            output = DataProto.from_dict(tensors={"values": values})
            output = self.ulysses_sharding_manager.postprocess_data(data=output)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def update_critic_prompt(self, data: DataProto):
        # Support all hardwares
        data = data.to(get_torch_device().current_device())
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.critic_module)
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_critic", logger=None) as timer:
                metrics = self.critic.update_critic_prompt(data=data)
            delta_time = timer.last
