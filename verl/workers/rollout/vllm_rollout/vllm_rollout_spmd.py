...
class vLLMRollout(BaseRollout):
    ...
    @GPUMemoryLogger(role="vllm rollout spmd", logger=logger)
    @torch.no_grad()
    def generate_sequences_agent(self, prompts: DataProto, **kwargs) -> DataProto:
        '''
        Active is a numpy array of bool, indicating which examples are active.
        '''
        ...
        active_masks = prompts.non_tensor_batch['active_masks']        
        idx = prompts.batch["input_ids"]  # (bs, prompt_length)
        # left-padded attention_mask
        attention_mask = prompts.batch["attention_mask"]
        position_ids = prompts.batch["position_ids"]
        ...
        with self.update_sampling_params(**kwargs):
            ...
            outputs = self.inference_engine.generate(
                ...
                sampling_params=self.sampling_params,
                lora_request=[_lora_request for _lora_request, active_mask in zip(lora_requests, active_masks) if active_mask] if lora_requests else None,
                use_tqdm=False,
            )                
