...
class DataParallelPPOCritic(BasePPOCritic):
    ...
    def _forward_prompt_micro_batch(self, micro_batch):
        response_length = micro_batch["responses"].size(-1)
        multi_modal_inputs = {}
        if "multi_modal_inputs" in micro_batch:
            for key in micro_batch["multi_modal_inputs"][0].keys():
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                values = pad_input(values_rmpad, indices=indices, batch=batch, seqlen=seqlen).squeeze(-1)
                ## only changes ##
                values = values[:, -response_length - 1]
            else:
                output = self.critic_module(
                    input_ids=input_ids,
                    attention_mask=attention_mask,
                ...
                values = output.logits
                ## only changes ##
                values = values[:, -response_length - 1].squeeze(-1)
            return values
        
    @GPUMemoryLogger(role="dp critic", logger=logger)
    def compute_prompt_values(self, data: DataProto) -> torch.Tensor:
        self.critic_module.eval()
        micro_batch_size = data.meta_info["micro_batch_size"]
        select_keys = ["responses", "input_ids", "attention_mask", "position_ids"]
        batch = data.select(batch_keys=select_keys).batch
        ...
        for micro_batch in micro_batches:
            ...
            with torch.no_grad():
                values = self._forward_prompt_micro_batch(micro_batch)
            values_lst.append(values)
        values = torch.concat(values_lst, dim=0)
        responses = data.batch["responses"]
        attention_mask = data.batch["attention_mask"]
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    vpreds = self._forward_prompt_micro_batch(data)
                    ...
                    vf_loss = clipped_vf_losses.mean()
                    vf_clipfrac = torch.gt(vf_losses2, vf_losses1).float().mean()
                    ...
                    data = {
                        ...
                        "critic/vpred_mean": vpreds.mean().detach().item(),
                    }
