...
def compute_policy_loss(
    ...
    clip_level: str = 'token', # 'token' or 'sequence'
    ...
    """
    assert clip_level in ['token', 'sequence'], f"clip_level should be 'token' or 'sequence', but got {clip_level}"
    ...
    if clip_level == 'token':
    ...
    else: # clip_level in ['gspo_token', 'sequence']
        # https://github.com/volcengine/verl/blob/main/verl/trainer/ppo/core_algos.py#L906
        if cliprange_low is None:
            cliprange_low = cliprange
        if cliprange_high is None:
            cliprange_high = cliprange
        clip_ratio_low, clip_ratio_high = cliprange_low, cliprange_high
        ...
        '''Like GSPO-token, the log_prob_diff is normalzied by sequence length'''
        # if clip_level == 'gspo_token':
        seq_lengths = torch.sum(response_mask, dim=-1).clamp(min=1)
        negative_approx_kl_seq = torch.sum(negative_approx_kl * response_mask, dim=-1) / seq_lengths
        # else:
        #     negative_approx_kl_seq = torch.sum(negative_approx_kl * response_mask, dim=-1)

        # Combined ratio at token level:
        # s_i,t(θ) = sg[s_i(θ)] · π_θ(y_i,t|x, y_i,<t) / sg[π_θ(y_i,t|x, y_i,<t)]
        # In log space: log(s_i,t(θ)) = sg[log(s_i(θ))] + log_prob - sg[log_prob]
        log_seq_importance_ratio = log_prob - log_prob.detach() + negative_approx_kl_seq.detach().unsqueeze(-1) 
        log_seq_importance_ratio = torch.clamp(log_seq_importance_ratio, max=10.0, min=-10.0)  # clamp for numerical stability
        ...
        if tis_imp_ratio_cap > 0 and rollout_log_probs is not None:
            # Apply truncated importance sampling -> https://fengyao.notion.site/off-policy-rl
            seq_log_ratio = (old_log_prob - rollout_log_probs) * response_mask
            tis_imp_ratio = torch.exp(seq_log_ratio.sum(dim=-1, keepdim=True))
            tis_imp_ratio = torch.clamp(tis_imp_ratio, max=tis_imp_ratio_cap)
            pg_losses = pg_losses * tis_imp_ratio
